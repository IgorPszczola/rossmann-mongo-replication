...
def _receipt_summary_fields(change):
    ...
    return (
        doc.get("transaction_id", "unknown"),
        customer.get("first_name", "unknown"),
        payment.get("amount_paid", "unknown"),
    )
...
def print_change_summary(change):
    ...
    if operation == "insert":
        transaction_id, customer_name, amount = _receipt_summary_fields(change)
    ...
    if operation == "update":
        ...
        print(
            f"Customer: {customer_name}, New Amount: {amount} PLN "
            f"(ID: {transaction_id})"
        )
        return
    ...
    if operation == "delete":
        print(
            f"RECEIPT DELETED! Operation: {operation}, "
        ...
        with stream:
            for change in stream:
                ...
                if replicated:
                    ...
                    print(
                        "MOVED to DLQ + replication offset updated. "
                        "Continuing with next change."
                    )
                print("-" * 50)
